...
abstract class AzlEasySSLModel
{
            ...
            foreach ($this->getAttributes() as $attr => $value) {
                if (($attributes && in_array($attr, $attributes)) || empty($attributes)) {
                    ...
                    if ($this->getMultiLangAttribute($attr)) {
                        ...
                        Configuration::updateValue($a, $values);
                    } else {
                                    ...
                                    if (Validate::$method($v)) {
                                        ...
                                        if ($addErrors) {
                                            if (isset($validator['message'])) {
                                                ...
                                                $this->addError($attr, sprintf($this->l('Incorrect "%s" value', 'AzlEasySSLModel'), $this->getAttributeLabel($attr)));
                                            }
                                        }
                                        $valid = false;
                                    }
            ...
            if ($this->getAttributeType($attr) == 'switch') {
                $config[$name]['values'] = array(
                    array(
                        'id' => 'active_on',
                        'value' => true,
                        'label' => $this->l('Enabled', 'AzlEasySSLModel'),
                    ),
                    array(
                        'id' => 'active_off',
                        'value' => false,
                        'label' => $this->l('Disabled', 'AzlEasySSLModel'),
                    )
                );
            }
